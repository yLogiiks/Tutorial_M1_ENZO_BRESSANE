...
func _process(delta):
	zoom = lerp(zoom, Vector2(1,1), 0.9)
	
	if iniciar_tremor_camera == true:
		global_position += Vector2(rand_range (-intensidade_tremor_camera, intensidade_tremor_camera), rand_range(-intensidade_tremor_camera, intensidade_tremor_camera)) * delta
